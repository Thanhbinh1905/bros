...
OpenCode BROS agents use pattern-based Bash permissions. `bro-build` defaults to explicit local inspection, validation, test, build, lint, typecheck, and package dry-run allowlists for implementation work, with unmatched Bash ask-gated and risky command classes explicitly ask- or deny-gated. Review, security, exploration, and orchestration agents remain narrower by default.

Optional BROS configuration can be supplied through `bros.config.json`. See `docs/configuration.md` for the full guide, `examples/bros.config.schema.json` for the schema, `examples/bros.config.example.json` for a complete example, and `examples/category-routing.md` for category semantics. The published schema is fetchable at `https://raw.githubusercontent.com/Thanhbinh1905/bros/main/examples/bros.config.schema.json`. The rich routing surface supports top-level `fallback_models`, `categories`, `routing_profiles`, `agents`, `permission_profiles`, and `approval_packages`; resolver precedence is `agents` > explicitly selected `routing_profiles.<depth>` > directly bound base `categories` > `fallback_models` > packaged/default agent config. Default OpenCode plugin startup does not infer per-message workflow depth. Category descriptions, capabilities, and workflow responsibilities are non-authoritative routing metadata only; permissions and approvals remain separate trusted surfaces. Removed `fallback_model` and `model_routing` keys are rejected.

Named workflow modes are `INFO_ONLY`, `DOC_ONLY`, `READ_ONLY_REVIEW`, `SMALL_PATCH`, and `FULL_BROS`. Depth profiles are `quick`, `standard`, `deep`, and `critical`. Governance tiers are `compact`, `standard`, and `full`; compact is valid only for safe non-routed or low-risk status/doc work, while full remains required for security, production, permissions, complex delivery, and reviewer conflict.

Expiry-bound approval packages can reduce repeated prompts for named local command classes such as `git_read`, `npm_local_dev`, `docker_local`, and `release_dry_run`, but hard denies remain authoritative for secrets, publish, dist-tag mutation, destructive reset/clean/delete, credential validation, protected branch push, force push, and production/cloud mutation. Approval package `files` are audit/reporting metadata only; runtime command permissions are package-preset based and are not constrained by those file globs.
...
- `bro-build`: may run explicitly allowlisted project inspection, git read-only inspection, local validation scripts, test/build/lint/typecheck commands, package dry-runs, Docker inspection, GitHub PR/run inspection, and localhost curl checks; unmatched Bash, edits, git mutation, dependency installs, Docker runtime/mutation, deploy/publish, and high-risk operations remain approval-gated or denied by task packet scope.

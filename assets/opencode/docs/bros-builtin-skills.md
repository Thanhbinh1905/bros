...
- Use the lightest safe routing mode and governance tier: `INFO_ONLY`, `DOC_ONLY`, `READ_ONLY_REVIEW`, `SMALL_PATCH`, or `FULL_BROS`; `compact`, `standard`, or `full`. Security, production, permissions, credentials, destructive work, release, and reviewer conflict require full governance.

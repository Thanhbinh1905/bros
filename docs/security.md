...
- Permission profiles only tune packaged BROS agent permissions at config time. They do not introduce top-level OpenCode `permission`, do not use broad `bash: allow`, and append hard denies for secret reads, provider credential mutation, npm publish/dist-tags, destructive delete/reset/clean operations, force push, and production/cloud mutation.
